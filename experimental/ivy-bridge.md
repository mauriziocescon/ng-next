...
### 6. Directive Forwarding (`forward: surface<T>()` / `@forward()`)
Allows consumer-supplied directives to "tunnel" through a component and land on an internal native element, without requiring global compiler knowledge. This is the runtime counterpart of the forwarding model in the readme (`forward: surface<T>()` plus a single `@forward()` marker).

- **Change Class:** Compiler + Runtime.
- **Analogy with fragments:** The pattern mirrors fragments. The compiler processing the *call site* (the consumer of the forwarding component) prepares a compile-time artifact — a recipe of directive defs and binding functions — and passes it across the component boundary. The forwarding component receives an opaque, typed directive payload and places it at its `@forward()` site. Just as `@render(frag(args))` executes a pre-compiled template function without the child knowing anything about its contents, an `ɵɵapplyForwardedDirectives`-style instruction executes a pre-compiled directive recipe without the forwarding component knowing which directives are in it.
- **Compile-time responsibility (call site):** When the compiler processes `<Button use:ripple() use:tooltip(message={msg()}) />`, it generates the full recipe: directive defs, initial binding values, and update-pass binding functions. The directive matching and validation Ivy does today via CSS-selector scanning at the first create pass has already been done — at compile time, at the call site.
- **The forward-surface contract:** The forwarding component declares its surface with `forward: surface<T>()` in its `component(config)`. This is the public directive-compatibility interface it exposes: the surface type `T` used for compile-time validation. For the native `@forward()` placement, the compiler resolves the actual element type through the Angular DSL `IntrinsicElements` map and checks `H ⊑ T` (see `ng-dsl-type-checking-spec.md` §6.2, D025). Because the surface is *declared* rather than read off the `@forward()` element, the component may promise a wider surface (`HTMLElement`) than the element it forwards to, keeping the internal tag out of its public API. Whether the component's runtime `TView` has knowledge of the arriving directive defs depends on how the payload is represented by the chosen implementation option below.
...
- **Implementation Options:**
  1. **Synthetic per-site directive range:** At the `@forward()` element, allocate normal directive slots for the forwarded defs when the component instance is created. This fits Ivy's existing TView/LView machinery best, but it means the forwarding component's shared `TView` cannot be completely static with respect to forwarded directives. It may need a per-call-site child/adapter `TView`, or a runtime extension to attach additional directive ranges to an existing TNode.
  2. **Side-storage model:** Keep the forwarding component's `TView` static and store forwarded directive records in per-`LView` side storage. This preserves independent compilation but requires parallel implementations of most directive-slot services listed above. It is a larger runtime change than a simple instruction.
  3. **Adapter view/component:** Lower the forwarded element into a generated adapter template owned by the call site's compilation unit. The adapter can include the exact directive defs in its `TView` while the forwarding component only renders a fragment-like insertion point. This shifts complexity toward compiler/linker output and may preserve more existing Ivy invariants.
- **Independent Compilation:** The type contract can remain independent: the call site only needs the forwarding component's declared surface type and forwarding metadata. Runtime independence is harder: the chosen implementation must define where directive defs, slots, host bindings, providers, outputs, and destroy hooks live.
- **Uniqueness across the boundary:** Directive uniqueness (D023) is per *resolved host element*, so a directive applied both locally at the `@forward()` element and forwarded from the call site collides on the same final element and is rejected (`ng-dsl-type-checking-spec.md` §7.1).
- **Delta from Ivy Today:** Current Ivy matches directives during the first create pass from `tView.directiveRegistry`; instances live in TView-indexed LView expando slots; providers and host bindings are registered from those slots. The recipe model moves selection/validation to the call-site compiler, but the runtime must still integrate the selected directives into Ivy's directive lifecycle machinery.

---

### 7. Hostless Scoped CSS
