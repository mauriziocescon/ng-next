...
- **The "Directive Forwarding" Contract (CompA):** The child defines directive forwarding metadata with `component.withDirectiveForwarding<T>(...)`. This is the public interface CompA exposes: the element type `T` for compile-time validation. Whether CompA's runtime `TView` has knowledge of the arriving directive defs depends on the chosen implementation option below.
...
- **Independent Compilation:** The type contract can remain independent: CompB only needs CompA's declared forwarded element type and forwarding metadata. Runtime independence is harder: the chosen implementation must define where directive defs, slots, host bindings, providers, outputs, and destroy hooks live.
...
| **Directives** | Automatically attach to the host element or match normal elements/containers from the compilation scope. | Consumer-supplied directives are compiled into recipes and forwarded to explicit `@forward()` sites. Runtime must still integrate them with directive slots or an equivalent host-binding/output/DI/destroy system. |
