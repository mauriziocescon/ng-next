...
let state: QuizState = createFreshState();
...
function answer(userSaidValid: boolean): void {
  ...
  if (q.valid) {
    ...
    if (!correct) btnInvalid?.classList.add('wrong');
  } else {
    btnInvalid?.classList.add('correct');
    if (!correct) btnValid?.classList.add('wrong');
  }
